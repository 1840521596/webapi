...
class JingPinKeCheng_Test(unittest.TestCase):
    ...
    def test_1_MRYJ_meiriyijugushiwen(self):
        """每日一句--《每日一句古诗文课程》课程信息-个人购买"""
        for project in self.productDict[u"每日一句"]:
            for course_info in project:
                if course_info["productName"] == u"每日一句古诗文课程":
                    ...
                    params = {"callback":"jQuery22405712752380829795_1557040396148","url":"https://resource.yunshuxie.com/dailySentence/sale/sale-daily-clock2018.html","_":"1557040396149"}
                    ...
                    # url = r"https://wap.yunshuxie.com/v1/video_public/get_token.htm"
                    # params = {"callback": "jQuery22405712752380829795_1557040396148", "vid": "eaceb0a5eaf05970eecfe1a194889bd0_e","_": "1557040396150"}
                    # self.resp = requests.get(url=url,headers=self.headers,params=params,cookies=self.cookies)  #获取token
                    # logging.info(url + lianjiefu + self.resp.text + fengefu)
                    # result = json.loads(re.findall("{.*}", self.resp.content)[0], encoding="utf8")
                    # assert result["returnCode"] == "0",self.msg.format(Except="0",Really=result["returnCode"])
                    ...
                    params = {"phone":self.phoneNum,"productId":"2846","phId":"2855",
                              "code":"o38sIv_7FQInsBKJEUExn7wYxoHc","sk":"null","agentId":"","customizeGroupId":"-1","activityId":"-1","cSn":""}
                    self.resp = requests.get(url=url,headers=self.headers,params=params ,cookies=self.cookies)   #生成支付订单
    ...
    def test_2_MRYJ_meiriyiduanzuowensucai(self):
        ...
        for project in self.productDict[u"每日一句"]:
            for course_info in project:
                if course_info["productName"] == u"每日一段作文素材课":
                    self.resp = requests.get(course_info["saleUrl"], headers=self.headers, cookies=self.cookies)
                    logging.info(course_info["saleUrl"] + lianjiefu + self.resp.text + fengefu)
            ...
            #  获取课程信息
                    ...
                    params = {"_":"1557217444577","callback":"Zepto1557217444367"}
                    self.resp = requests.get(url=url,headers=self.headers,params=params,cookies=self.cookies)
                    logging.info(url + lianjiefu + self.resp.text + fengefu)
                    result = json.loads(re.findall("{.*}", self.resp.content)[0], encoding="utf8")
                    productHoursId = result["data"]["hours"][0]["productHoursId"]  # 选择课程，默认第一个
                    productId = result["data"]["hours"][0]["productId"]  # 选择课程，默认第一个
                    assert result["returnCode"] == "0"
                    url = r"https://pay.yunshuxie.com/v6/order/query/wm_is_pay.htm"
                    params = {"phone":self.phoneNum,"productCourseHourId":productHoursId,"callback":"__jp6"}
                    self.resp = requests.get(url=url, headers=self.headers, params=params, cookies=self.cookies)
                    logging.info(url + lianjiefu + self.resp.text + fengefu)
                    result = json.loads(re.findall("{.*}", self.resp.content)[0], encoding="utf8")
                    assert result["returnCode"] == "0"
                    ...
                    params = {"phone": "{}".format(self.phoneNum), "customizeGroupId": "-1", "ysxOpenId": "osxBJ6MQ69yOMyhCejqj55SdKzyI", "phId": productHoursId,
                    ...
                    result =json.loads(self.resp.content,encoding="utf8")
                    assert result["returnCode"] == "0", self.msg.format(Except="0", Really=result["returnCode"])
    def test_3_MRYJ_kantuxiehua60jiang(self):
        """每日一句 - -《看图写话60讲》课程信息"""
        for project in self.productDict[u"每日一句"]:
            for course_info in project:
                if course_info["productName"] == u"看图写话60讲":
                    self.resp = requests.get(course_info["saleUrl"], headers=self.headers, cookies=self.cookies)
                    logging.info(course_info["saleUrl"] + lianjiefu + self.resp.text + fengefu)
                    productPosters_resp = requests.get(course_info["productPosters"], headers=self.headers,
                                                       cookies=self.cookies)
                    ...
                    params = {"_":"1557200779939","callback":"Zepto1557200779894"}
                    self.resp = requests.get(url=url, headers=self.headers, params=params, cookies=self.cookies)
                    logging.info(url + lianjiefu + self.resp.text + fengefu)
                    result = json.loads(re.findall("{.*}", self.resp.content)[0], encoding="utf8")
                    ...
                    params = {"cType":"73","_":"1557209606711","callback":"Zepto1557209606555"}
                    self.resp = requests.get(url=url, headers=self.headers, params=params, cookies=self.cookies)  #
                    logging.info(url + lianjiefu + self.resp.text + fengefu)
                    result = json.loads(re.findall("{.*}", self.resp.content)[0], encoding="utf8")
                    ...
                    signature_jsonp_resp = requests.get(url=url, params=params, headers=self.headers, cookies=self.cookies)  # 生成支付订单
                    logging.info(url + lianjiefu + signature_jsonp_resp.text + fengefu)
                    url = r"https://account.yunshuxie.com/v1/validate/wap/newplat_code_reset.htm?phone={phone}&type=2".format(phone=self.phoneNum)
                    account_resp = requests.get(url=url,headers=self.headers,cookies=self.cookies)  # 获取验证码，自动完成{"msg":"验证码为123456"}
                    logging.info(url + lianjiefu + account_resp.text + fengefu)

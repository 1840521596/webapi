...
class BearWord_Teacher_Test(unittest.TestCase):
    """<br>首页，不需要账号<br>1.admin查询用户手机号<br>2.admin添加用户手机号为教师<br>3.APP个人中心-开启通知并判断是否教师角色<br>4.APP个人中心关闭接收作业<br>5.APP个人中心开启接收作业<br>6.老师端：待批改列表<br>7.老师端：用户作业详情<br>8.驳回理由列表<br>9.老师端：校验用户是否重新提交作业<br>10.上传批改语音接口<br>11.老师端：批改作业保存<br>12.老师端：批改记录<br>13.老师端：推荐/取消推荐优秀作业-推荐<br>14.老师端：推荐/取消推荐优秀作业-取消推荐"""
    @classmethod
    def setUpClass(self):
        self.redis = MyRedis()
    ...
    def test_02_admin_bear_course_add_bearTeacher(self):
        ...
        if bearWord_Teacher_memberId:
            ...
            header = {"Accept": "application/json, text/javascript, */*; q=0.01",
                      ...
                      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36",
                      "X-Requested-With": "XMLHttpRequest"}
            params = {"memberId":bearWord_Teacher_memberId}
            logging.info(url + lianjiefu + json.dumps(params, ensure_ascii=False) + fengefu)
            str_params = json.dumps(params, ensure_ascii=False, encoding="utf8")
            print str_params
            self.resp = requests.get(url=url, params=params,headers=header,cookies=cookies)
    ...
    def test_05_v1_bear_teacher_bear_teacher(self):
        ...
        if result["returnCode"] == "0" or result["returnCode"] == 0:
            ...
            assert result["returnCode"] == expect["returnCode"], self.msg.format(Expect=expect["returnCode"],
                                                                                 Really=result["returnCode"])
    def test_06_v1_bear_teacher_not_correct_list(self):
    ...
    def test_07_v1_bear_teacher_bear_task(self):
        ...
        if bearWord_timelineId:
            ...
            if result["data"]["notCorrectJobList"]:
                ...
                if result["data"]["notCorrectJobList"][0]["submitUpdateDate"] !="":
                    bearWord_submitUpdateDate = self.redis.str_set("bearWord_submitUpdateDate",result["data"]["notCorrectJobList"][0]["submitUpdateDate"])
        else:
            print u"当前教师未存在待批改作业"
            raise Exception,u"当前教师未存在待批改作业"
    def test_08_v1_bear_teacher_get_reject_reason_list(self):
        ...
        if result["returnCode"] == "0" or result["returnCode"] == 0:
            ...
            assert result["returnCode"] == expect["returnCode"], self.msg.format(Expect=expect["returnCode"],
                                                                                     Really=result["returnCode"])
    def test_09_v1_bear_teacher_check_timeLineSatus(self):
        ...
        if bearWord_timelineId:
            ...
            print u"当前教师未存在待批改作业"
            raise Exception, u"当前教师未存在待批改作业"
    def test_10_v1_bear_teacher_upload_voice(self):
    ...
    def test_11_v1_bear_teacher_save_correction_records(self):
        ...
        if bearWord_timelineId and commentVoice:
            ...
            print u"当前教师未存在待批改作业"
            raise Exception, u"当前教师未存在待批改作业"
    def test_12_v1_bear_teacher_corrected_record_list(self):
        ...
        if result["returnCode"] == "0" or result["returnCode"] == 0:
            ...
            assert result["returnCode"] == expect["returnCode"], self.msg.format(Expect=expect["returnCode"],
                                                                                 Really=result["returnCode"])
    def test_13_v1_bear_teacher_recommend_job(self):
        ...
        if bearWord_timelineId:
            ...
            print u"当前教师未存在待批改作业"
            raise Exception,u"当前教师未存在待批改作业"
    def test_14_v1_bear_teacher_recommend_job(self):
